...
1. Éditer `frontend/src/config/site.ts` (défaut = Audiar / Rennes Métropole) : nom, codes INSEE, centre carte, mentions.
...
4. Fournir un WFS (`GEOSERVER_URL`) **ou** un GeoJSON (`COMMUNES_GEOJSON_PATH`) dont les `code_insee_concat` matchent `site.ts`.
